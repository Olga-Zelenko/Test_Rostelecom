...
def test_page_left_registration(driver):
    """EXP-11 Продуктовый слоган ЛК "Ростелеком ID" располагается в правой части страницы «Регистрация»."""
    try:
        page_reg = RegistrPage(driver)
        assert 'Персональный помощник в цифровом мире Ростелекома' not in page_reg.page_left_registration.text
    except AssertionError:
        print('Элемент отсутствует в левой части формы')


def test_elements_of_registr(driver):
    """EXP-12 Проверка формы регистрации на наличие основных элементов.
    - поля ввода:
        Имя,
        Фамилия,
        Регион,
        email или мобильный телефон,
        Пароль,
...
def test_names_elements_of_registr(driver):
    """EXP-13 Названия элементов блока регистрации формы «Регистрация» соответствуют Требованию."""
    try:
        page_reg = RegistrPage(driver)
...
def test_registr_by_valid_data(driver):
    """EXP-14 Регистрация пользователя с валидными данными (необходимо каждый раз использовать
    незарегистрированный ранее email)"""
    page_reg = RegistrPage(driver)
...
def test_registr_by_invalid_data(driver):
    """EXP-15 Регистрация пользователя по email, который уже был использован ранее для регистрации"""
    page_reg = RegistrPage(driver)
    page_reg.first_name.send_keys(Settings.f_name)
    page_reg.last_name.send_keys(Settings.l_name)
...
def test_registr_and_redir_auth(driver):
    """EXP-16 Попытка регистрации пользователя по email, который уже был использован ранее для регистрации,
    и нажатие на кнопку 'Войти' для редиректа в форму "Авторизация". """
    page_reg = RegistrPage(driver)
    page_reg.first_name.send_keys(Settings.f_name)
    page_reg.last_name.send_keys(Settings.l_name)
    page_reg.email_registration.send_keys(Settings.valid_email)
...
def test_first_name_by_valid_data(driver, valid_first_name):
    """EXP-17 Проверка поля ввода "Имя" формы «Регистрация» валидными данными:
...
def test_first_name_by_invalid_data(driver, invalid_first_name):
    """EXP-18 Проверка поля ввода "Имя" формы «Регистрация» невалидными данными:
    пустое значение;
...
def test_last_name_by_valid_data(driver, valid_password):
    """EXP-19 Проверка поля ввода "Пароль" формы «Регистрация» валидными данными:
...
def test_passw_registration_confirm_valid_data(driver):
    """EXP-20 Проверка поля ввода "Пароль" и "Подтвердить пароль" формы «Регистрация» валидными данными
    (пароли совпадают)."""
    page_reg = RegistrPage(driver)
...
def test_passw_registration_confirm_invalid_data(driver):
    """EXP-21 Проверка поля ввода "Пароль" и "Подтвердить пароль" формы «Регистрация» невалидными данными
    (пароли не совпадают)."""
    page_reg = RegistrPage(driver)
    page_reg.passw_registration.send_keys(Settings.passw1)
